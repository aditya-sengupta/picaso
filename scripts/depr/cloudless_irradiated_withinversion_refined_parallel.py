...
def run(grav, teff, semi_major):
    ...
    try:
        ...
        print(f"[{grav}, {teff}, {semi_major}] Starting at nstr_upper = {nstr_upper}")
        ...
        print(f"[{grav}, {teff}, {semi_major}] ✗ Error: {e}")
        gc.collect()
        return False
...
def run_through_loop(num_threads=10):
    ...
    for i, (grav, teff, semi_major) in enumerate(tasks):
        # Only process tasks assigned to this rank
        if i % size == rank:
            result = run(grav, teff, semi_major)
            if result:
                local_completed += 1
            else:
                local_failed += 1
