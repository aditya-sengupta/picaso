...
cmap = cm.RdBu(np.linspace(0, 1, len(semimajors)+1))[:-1]
for (j, cloudmode) in enumerate(cloudmodes):
    for (i, grav) in enumerate(gravs):
        for (semimajor, c) in zip(semimajors, cmap):
            ...
            for teff in teffs:
                ...
                elif teff >= 900 and grav >= 31:
                    if cloudmode == "fixed":
                        cl = "f2"
                    else:
                        cl = "nc"
                    sonora_df = pd.read_csv(f"reference/sonora_grids/diamondback/t{teff}g{grav}{cl}_m0.0_co1.0.pt", sep=r"\s+", skiprows=[1])
                    pressure_sonora = np.array(sonora_df["P"])
                    temp_sonora = np.array(sonora_df["T"])
                    t10s.append(t10(pressure_sonora, temp_sonora))
                    teffs_this.append(teff)

            label = f"a = {semimajor} au" if semimajor < np.inf else "diamondback"
            axs[j,i].plot(teffs_this, t10s, c=c, label=label)
