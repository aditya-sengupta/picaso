            ...
            if semimajor < np.inf:
                ...
                if os.path.exists(os.path.join(picaso_path, fname)):
                    with h5py.File(os.path.join(picaso_path, fname)) as f:
                        ...
                        if cvz_locs[4] > 0 and cvz_locs[5] > cvz_locs[2]:
                ...
                teffs_this.append(teff)
                p, _ = diamondback_pt(teff, grav, "nc")
                rcbs.append(p[find_rcb_diamondback(teff, grav, "nc")])
            # add in Diamondback 
...
figpath = os.path.join(picaso_path, "figures/rcb/rcb_table.pdf")
plt.savefig(figpath)
print(figpath)
plt.close(fig)