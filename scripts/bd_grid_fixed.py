...
for semi_major in [np.inf]:
    for teff in [900, 1400, 1700, 1900]:
        print(f"effective temperature = {teff} K, semimajor = {semi_major} au")
        ...
        sonora_df_cloudy = pd.read_csv(f"reference/sonora_grids/diamondback/t{teff}g{grav}f{fsed}_m0.0_co1.0.pt", sep=r"\s+", skiprows=[1])
        ...
        try:
            ...
            with h5py.File(f"data/bd_fixed/bd_fsed{fsed}_teff{teff}_semimajor_{semi_major}_cloudmode{cloudmode}_dt{tstamp}.h5", "w") as f:
            ...
            with open(f"./data/bd_fixed/fsed{fsed}_teff{teff}_semimajor{semi_major}_dt{tstamp}.txt", "w") as f:
